...
courses = cv.get('/courses', enrollment_state='active', include=['term'])
# Filter out clearly inactive: courses with no recent term, or term ended >7 days ago
```

CC presents course **names only** to the student. No course IDs, no API field names.
...
> - ...
>
> Which ones should I track? Default is all.

CC waits for: a selection. Accept "all" / "全部" / silence (= all) / a list of names ("just the first three" / "前两个和最后一个" / "skip the writing one"). For ambiguous input ("the easy ones"), ask one clarifying question, don't guess.
